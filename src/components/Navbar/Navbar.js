...
export default function Navbar({showNotif}) {
        ...
        <div className="container ">
          <div className="navbar-content		flex justify-between items-start pt-4 ">
              ...
              <div className=" 	w-full pt-2 ">
                ...
                >
                  <li className="flex  p-2 rounded-t	mx-auto">
                    <Accordion className="w-[200px]" defaultActiveKey="1">
                      {categories.map((category,index)=>(

                      <Accordion.Item key={category._id } className="bg-bg " eventKey={index+1}>
                        ...
                        {category.subcategories ? (
                          ...
                          <Link key={sub._id} to={`/shop/${sub.href}`}>{sub.title}</Link>
                            ))}
                        </Accordion.Body>
                        ):(<></>)}
                  ...
                  <li className="flex    rounded-t w-[200px] mx-auto	">
                    <a
                      className="text-bg bg-text cursor-default	  w-full mt-3 p-2 rounded-lg text-center"
                      href="#"
                    >
                      نصب تاجک
                    </a>
              ...
              <div className=" 	w-full pt-2">
                <ul className="  flex font-bold text-lg  justify-between w-full 	">
                  <li
                    ...
                    className="flex hover:bg-bg p-2 rounded-t	cursor-default	"
                  >
                    <svg
                      width="24"
                      height="24"
                    ...
                    >
                      <g strokeWidth="url(#clip0_926_2914)">
                        <path
                    ...
                    <ul
                      ...
                      } transition-all duration-300 flex justify-around  flex-wrap absolute top-full bg-bg md:left-[20%] lg:left-[30%] xl:left-[29.5%] min-w-[500px] max-w-[600px] gap-5 h-[400px]  border border-gray-300 rounded-b-lg p-2 shadow-lg transform -translate-y-2 `}
                    >
                      {categories.map((category) =>(
                        <li key={category._id} className="text-gmain font-bold text-xl">
                        {category.title}
                        <ul>
                          {category.subcategories ? (
                            
                            <li className="text-text font-normal	flex flex-col">
                            {category.subcategories.map((sub) =>(
                              <Link key={sub._id} to={`/shop/${sub.href}`}>{sub.title}</Link>
                            ))}
                          </li>
                  ...
                  <li className="">
                    ...
                    <svg
                      ...
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <g strokeWidth="url(#clip0_926_2900)">
                        <path
                  ...
                  <li className="flex hover:bg-bg p-2 rounded-t	cursor-default	">
                    <svg
                      width="26"
                      height="26"
                      viewBox="0 0 26 26"
            ...
            <div className="navbar-left flex justify-around items-center relative  ">
              <NavLink to='/basket'>
                <img
                  className="relative	 w-6 h-6 md:w-10 md:h-10 ml-3"
                  src="/images/Frame 365.png"
                  alt=""
                />
                {products.length ? (
                  <span className="absolute	w-4 h-4 lg:w-5 lg:h-5 rounded-full bg-orange-400 top-0 -right-2 flex items-center justify-center">{products.length}</span>
                ) : (
                  ''
                )}
              </NavLink>
              {authContext.userInfos.username ? (
                <div className="relative flex items-center">
                  {authContext.userInfos.role === 'ADMIN' ? (
                    <>
                    <button className="w-6 h-6 md:w-10 md:h-10 md:absolute	md:-right-2">
                    <img
                    ...
                    <button
                      className="hidden md:block  border-t border-b border-gmain rounded-lg w-40 h-10 font-bold ">
                      <Link to='/p-admin'>
                      {authContext.userInfos.username}
                      </Link>
                    </button>
                ...
                <div className="relative flex items-center">
                <Link to='/login'  className="w-6 h-6 md:w-10 md:h-10 md:absolute	md:-right-2">
                <img
                  className=" w-6 h-6 md:w-10 md:h-10 "
                  src="/images/profile.svg"
                  alt=""
              ...
              <div ref={profilRef} className={`${showProfile ? 'block' : 'hidden'} menu-profile w-[212px] h-auto bg-bg absolute	top-[50px] left-0 shadow-lg rounded-lg`}>
                <ul className="p-4">
                  <li className="flex items-center justify-between">
                    <NavLink to='/my-account/edit' className="font-bold text-text" href="">{authContext.userInfos.username}</NavLink>
                    <svg
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                  ...
                  <li className="flex items-center justify-between">
                    ...
                    <button onClick={logOutHandeler} className="font-bold text-text" >خروج از حساب</button>
                  </li>
                </ul>
              </div>
            </div>
