...
export default function ProductBox({data}) {
  ...
  const discountBtn =() =>{
    ...
    console.log( discount)
        fetch(`https://tajak-project.liara.run/api/product/${data._id}/discount`, {
          method: "PUT",
          headers: {
  ...
  const OfferDayBtn = () =>{
    ...
    const localData = JSON.parse(localStorage.getItem("user"));
        ...
        fetch(`https://tajak-project.liara.run/api/offer/${data._id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localData.token}`,
          },
          body: JSON.stringify(newOfferDay)
    ...
    <div className="productbox p-3 border-solid border-gmain border-y-2 rounded-lg flex items-center justify-between">
      <div className="right flex items-center justify gap-2">
        ...
        <div className="name text-text font-bold">
          <h1>{data.name}</h1>
          <h1>{data.authorID.name}</h1>
        </div>
      </div>
      <div className="middle flex flex-col items-center justify-center">
        <span className="price font-bold">{data.printed[0].pricePrinted} تومان</span>
          ...
          <div className="modal-middel mt-5">
            <div className=" content-discount bg-bg p-3 flex flex-col items-center justify-center flex-wrap  gap-3 rounded-lg">
              <div className="btns-discount flex gap-4">
                <button onClick={(event) =>setDiscount(5)} className="w-[64px] h-10 border-solid border-red border-2 rounded-lg ">
                  5%
                </button>
                <button onClick={(event) =>setDiscount(10)} className="w-[64px] h-10 border-solid border-red border-2 rounded-lg ">
                  10%
                </button>
                <button onClick={(event) =>setDiscount(20)} className="w-[64px] h-10 border-solid border-red border-2 rounded-lg ">
                  20%
                </button>
                <button onClick={(event) =>setDiscount(25)} className="w-[64px] h-10 border-solid border-red border-2 rounded-lg ">
                  25%
                </button>
                <button onClick={(event) =>setDiscount(50)} className="w-[64px] h-10 border-solid border-red border-2 rounded-lg ">
                  50%
                </button>
              </div>
              ...
              <div className="add mt-3">
                <button onClick={discountBtn} className="w-[150px] h-10 bg-gmain text-bg rounded-lg font-bold">
                  ثبت تخفیف
                </button>
              </div>
            </div>
          ...
          <div className="modal-middel mt-5">
            <div className=" content-offer bg-bg p-3 flex flex-col items-center justify-center flex-wrap  gap-3 rounded-lg">
              <div className="input mt-3">
                ...
                <div className=" relative mt-2">
                  <input
                  onChange={(event) => setSubtitle3(event.target.value)}
                    className="w-[270px] h-10 border-solid border-gmain border-1 p-1 rounded-lg bg-bg 	"
                    type="text"
                  />
                </div>
              ...
              <div className="add mt-3">
                <button onClick={OfferDayBtn} className="w-[150px] h-10 bg-gmain text-bg rounded-lg font-bold">
                  ثبت 
                </button>
              </div>
            </div>
